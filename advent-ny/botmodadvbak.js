...
const aviableDatesArray = [button1];
// date2,
// date3,
// date4,
// date5,
...
const actionKbd = {
  reply_markup: JSON.stringify({
    keyboard: [[{ text: `${button1}`, callback_data: 'datesKbdCB' }]],
...
////////guard regex ///////////////
const regexGuard = /^[a-zA-Z0-9а-яА-Я]+$/;
///////////////////////////////////
...
mainScreen.enter(async (ctx) => {
  ...
  await mainScreen.on('message', async (ctx) => {
    // if (!regexGuard.test(ctx.message.text)) {
    //   await ctx.reply('!RESTRICTED SYMBOLS');
    //   await ctx.scene.reenter();
    // } else {
    // await ctx.reply('! REQUEST ACCEPTED');
    // Handle a command that triggers the POST request
    // async (ctx) => {
    const data = await axios
      ...
      .then((data) => {
        ...
        ctx.reply(JSON.stringify(`полученные данные ${data.data.answer}`));
      })
      .catch((error) => console.log(error));

    // const transformedData = JSON.stringify(data);
    // await ctx.reply('gggg');
    //
    ...
    //     ctx.reply('POST request successful!');
    //   })
    //   .catch((error) => {
    //     console.error(error);
    //     ctx.reply('Error occurred while making the POST request');
    //   });
    // };
    ...
    //   //  await ctx.scene.enter('SCENE_24_12');
    // } else {
    //   await ctx.reply(`${ctx.message.text} !NOT RECOGNIZED`);
    //   await ctx.scene.enter('MAIN_SCREEN_SCENE');
    // }
    // const res = isAdventPresentAviable(ctx.message.text, forcedTimeSet);
    // await ctx.reply(`isPresentAviable ${res}`);
